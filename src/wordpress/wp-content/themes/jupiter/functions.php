...
add_action( 'rest_api_init', 'sc_register_seo' );
function sc_register_seo () {
    register_rest_field( ['page', 'post'], 
    'seo_meta',
    array(
        'get_callback'    => 'sc_get_page_seo_meta',
        'update_callback' => null,
        'schema'          => null,
    ));
}

function sc_get_page_seo_meta( $object, $post ) {
    $psp_status = get_post_meta( $object[ 'id' ], 'psp_status' );

    return [
        'title' => $psp_status[0]['title']['debug']['str'],
        'description' => $psp_status[0]['meta_description']['debug']['str']
    ];
}

function rest_prepare_post_page( $data, $post, $request ) {
	$_data = $data->data;
    // echo html_entity_decode($_data['title']['rendered']);
    // exit;
    return [
        // 'request' => $_data,
        'id' => $_data['id'],
        'title' => html_entity_decode($_data['title']['rendered']),
        'link' => $_data['link'],
        'seo_meta' => $_data['seo_meta'],
        'image' => wp_get_attachment_url($_data['featured_media'])
    ];

}

add_filter( 'rest_prepare_page', 'rest_prepare_post_page', 0, 3 );
add_filter( 'rest_prepare_post', 'rest_prepare_post_page', 0, 3 );

function searchPageByMeta ( $args, $request ) {
    // cannot search via post content because we use visual composer
    $args['meta_key'] = 'psp_meta';

    $args['meta_query'] = [
        'relation' => 'AND',
        [
            'key' => 'psp_meta',
            'value' => $request->get_params()['country'],
            'compare' => 'LIKE'
        ],
        [
            'key' => 'psp_meta',
            'value' => $request->get_params()['blog_relation'],
            'compare' => 'LIKE'
        ]
    ];
...
function searchPostByMeta ( $args, $request ) {
    // cannot search via post content because we use visual composer
    $args['meta_key'] = 'psp_meta';
...
add_filter('rest_post_query', 'searchPostByMeta', 10, 2);
add_filter('rest_page_query', 'searchPageByMeta', 10, 2);

// change the attachment url 
add_filter('wp_get_attachment_url', 'sc_local_to_s3');
function sc_local_to_s3 ( $url ) {

    if (strpos($url, get_home_url()) !== false) {
        $url_parts = explode('/uploads/', $url);
        $url = 'https://s3-eu-west-2.amazonaws.com/sc30/uploads/' . $url_parts[1];
    }

    return $url;
}

add_filter('wp_calculate_image_srcset','sc_srcset_local_to_s3');

function sc_srcset_local_to_s3 ($sources) {
    $new_sources = [];
