...
class WPFEPP_CopyScape
{
	...
	function __construct($version, $textdomain) {
	...
	public function add_actions(){
		...
		foreach ( $post_types as $key => $type ) {
			if($this->options['column_types'][$type])
	...
	public function passed($post_data){
		...
		if(!$username || !$key)
			return new WP_Error( 'copyscape_error', __('You need to specify your CopyScape API keys in the plugin settings', $this->textdomain) );
