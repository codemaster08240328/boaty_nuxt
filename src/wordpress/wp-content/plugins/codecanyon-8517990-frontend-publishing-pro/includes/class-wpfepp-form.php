...
class WPFEPP_Form
{
	 ...
	 **/
	public function __construct($version, $textdomain, $form_id = -1)
	{
		...
		$this->captcha = new WPFEPP_Captcha($this->version, $this->textdomain);
	...
	public function display( $post_id = -1 ){
		...
		if(!$this->valid){
			_e("No form with the specified ID was found", $this->textdomain);
			return;
		}

		if(!is_user_logged_in()){
			printf(__('You need to %s first.', $this->textdomain), sprintf('<a href="%s">%s</a>', wp_login_url(), __('log in', $this->textdomain)));
			return;
		}
	...
	public function save_draft($post_data, $error_format = 'html') {
		...
		do{
			if(!$this->settings['enable_drafts']){
				$return_val['errors']['form'][] = __('Drafts are not allowed!', $this->textdomain);
				break;
			}
			...
			if($captcha_enabled && $this->captcha->keys_available() && $this->post_status($post_data['post_id']) == 'new'){
				...
				if(!$captcha_check){
					$return_val['errors']['form'][] = __('Captcha response incorrect', $this->textdomain);
					break;
				}
			}
			...
			$preview_link = sprintf('<br/><a target="_blank" href="%s">%s</a>', WPFEPP_Post_Previews::make_preview_link($result), __('Preview', $this->textdomain));
			$preview_link = ($this->post_type_obj->public) ? $preview_link : '';

			$return_val['errors']['form'][] = sprintf(
					__('The post has been saved successfully! %s', $this->textdomain),
					$preview_link
				);
	...
	public function handle_submission($post_data, $error_format = 'html'){
		...
		do {
			if(!$this->valid){
				$return_val['errors']['form'][] = __('This form no longer exists.', $this->textdomain);
				break;
			}

			if( !wp_verify_nonce($post_data['_wpnonce'], 'wpfepp-form-'.$post_data['form_id'].'-nonce') ){
				$return_val['errors']['form'][] = __('You failed the security check.', $this->textdomain);
				break;
			}
			if( $post_data['post_id'] != -1 && !$this->current_user_can_edit($post_data['post_id']) ){
				$return_val['errors']['form'][] = __('You do not have permission to modify this post.', $this->textdomain);
				break;
			}
			...
			$copyscape 			= new WPFEPP_CopyScape($this->version, $this->textdomain);
			...
			if($copyscape_enabled){
				...
				if(is_wp_error($passed)){
					$column_msg = __('ERROR: ', $this->textdomain) . $passed->get_error_message();
					$passed 	= true;
				}
				else {
					$column_msg 	= ($passed) ? __('passed', $this->textdomain) : __('failed', $this->textdomain);
				}
			...
			$preview_link 			= sprintf('<a target="_blank" href="%s">%s</a>', WPFEPP_Post_Previews::make_preview_link($result), __('Preview', $this->textdomain));
			$permalink 				= sprintf('<a target="_blank" href="%s">%s</a>', get_post_permalink($result), __('View', $this->textdomain));
			$final_link 			= ($post_data['post_status'] == 'publish') ? $permalink : $preview_link;
			$final_link 			= ($this->post_type_obj->public) ? $final_link : '';

			$return_val['errors']['form'][] = sprintf(
				__("The post has been %s successfully. %s %s", $this->textdomain),
				$action,
				sprintf('<br/><a class="wpfepp-continue-editing" href="#">%s</a>', __('Continue Editing', $this->textdomain)),
				$final_link
			);
