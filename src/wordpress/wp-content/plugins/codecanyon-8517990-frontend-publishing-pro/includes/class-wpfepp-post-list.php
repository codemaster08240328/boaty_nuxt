...
class WPFEPP_Post_List
{
	...
	public function __construct($version, $textdomain, $form_id = -1)
	{
		...
		$this->form = new WPFEPP_Form($this->version, $this->textdomain, $form_id);
	...
	public function handle_deletion_request(){
		if( isset($_GET['wpfepp_post']) && isset($_GET['wpfepp_action']) && is_numeric($_GET['wpfepp_post']) && isset($_GET['_wpnonce']) && $_GET['wpfepp_action'] == 'delete' ){
			...
			$sendback 		= add_query_arg( array_merge($blog_page, $success_vars), '' );
	...
	private function delete_post($post_id, $delete_nonce){
		...
		do{
			if(!wp_verify_nonce($delete_nonce, 'wpfepp-delete-post-'.$post_id.'-nonce')){
				$data['message'] = __('Sorry! You failed the security check', $this->textdomain);
				break;
			}	

			if(!$this->current_user_can_delete($post_id)){
				$data['message'] = __("You don't have permission to delete this post", $this->textdomain);
				break;
			}

			$result = wp_delete_post( $post_id, false );
			if(!$result){
				$data['message'] = __("The article could not be deleted", $this->textdomain);
				break;
			}

			$data['success'] = true;
			$data['message'] = __('The article has been deleted successfully!', $this->textdomain);
		}
		while (0);
