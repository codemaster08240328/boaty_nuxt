...
class Frontend_Publishing_Pro
{
	 ...
	 **/
	public function __construct($version = "1.0", $textdomain = "wpfepp-plugin")
	{
		...
		$this->shortcode_manager 	= new WPFEPP_Shortcode_Manager( $version, $textdomain );
		$this->form_manager 		= new WPFEPP_Form_Manager( $version, $textdomain );
		$this->plugin_settings 		= new WPFEPP_Plugin_Settings( $version, $textdomain, $this->form_manager->get_page_slug() );
		$this->admin_messages 		= new WPFEPP_Admin_Messages( $version, $textdomain );
		$this->email_manager 		= new WPFEPP_Email_Manager( $version, $textdomain );
		$this->form_ajax 			= new WPFEPP_Form_Ajax( $version, $textdomain );
		$this->post_list 			= new WPFEPP_Post_List( $version, $textdomain );
		$this->db 					= WPFEPP_DB_Table::get_instance();
		$this->copyscape 			= new WPFEPP_CopyScape($this->version, $this->textdomain);
...
JS;
	...
	public function upload_size($file) {
	    ...
	    if($size > $max_upload_size * 1024) {
	       $file['error'] = sprintf(__("Files larger than %s kilobytes are not allowed.", $this->textdomain), $max_upload_size);
	    }
	    return $file;
	}
