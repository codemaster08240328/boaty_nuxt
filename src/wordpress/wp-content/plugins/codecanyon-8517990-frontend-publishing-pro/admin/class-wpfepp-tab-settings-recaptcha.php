...
class WPFEPP_Tab_Settings_ReCaptcha extends WPFEPP_Tab
{
	...
	public function create_options(){
		...
		add_settings_section( $section, __('ReCaptcha', $this->textdomain), array($this, 'section_callback'), $tab );

		register_setting( $tab, $option_id, array($this->validator, 'validate') );

	    add_settings_field(
	        'site_key', __('Site Key', $this->textdomain), $callback, $tab, $section,
	        array_merge(
				array(
					'id' 	=> 'site_key',
					'desc' 	=> sprintf(__('Your ReCaptcha site key. You can get this key %s.', $this->textdomain), sprintf('<a href="http://www.google.com/recaptcha/admin">%s</a>', __('here', $this->textdomain))),
					'type' 	=> 'text'
				),
				$args
			)
	    ...
	    add_settings_field(
	        'secret', __('Secret', $this->textdomain), $callback, $tab, $section,
	        array_merge(
				array(
					'desc' 	=> sprintf(__('Your ReCaptcha secret. You can get it %s.', $this->textdomain), sprintf('<a href="http://www.google.com/recaptcha/admin">%s</a>', __('here', $this->textdomain))),
					'id' 	=> 'secret',
					'type' 	=> 'text'
				),
				$args
		...
		add_settings_field(
	        'theme', __('Theme', $this->textdomain), $callback, $tab, $section,
	        array_merge(
				array(
					'id' 	=> 'theme',
					'desc' 	=> __('Color scheme of the ReCaptcha widget.', $this->textdomain),
					'type' 	=> 'select',
	...
	public function section_callback($args){
		?>
			<p class="description"><?php _e('In this section you can manage your ReCaptcha API keys and settings.', $this->textdomain); ?></p>
		<?php
	}
}