...
class WPFEPP_Tab_Settings_CopyScape extends WPFEPP_Tab
{
	...
	public function create_options(){
		...
		add_settings_section( $section, __('CopyScape', $this->textdomain), array($this, 'section_callback'), $tab );

		register_setting( $tab, $option_id, array($this->validator, 'validate') );

	    add_settings_field(
	        'username', __('Username', $this->textdomain), $callback, $tab, $section,
	        array_merge(
				array(
					'id' 	=> 'username',
					'desc' 	=> __('Your copyscape username.', $this->textdomain),
					'type' 	=> 'text'
				),
				$args
			)
	    ...
	    add_settings_field(
	        'api_key', __('API Key', $this->textdomain), $callback, $tab, $section,
	        array_merge(
				array(
					'desc' 	=> sprintf(__('Your copyscape API key. You can get this key %s.', $this->textdomain), sprintf('<a href="http://www.copyscape.com/apiconfigure.php#key">%s</a>', __('here', $this->textdomain))),
					'id' 	=> 'api_key',
					'type' 	=> 'text'
				),
				$args
		...
		add_settings_field(
	        'block', __('Do not accept unoriginal submissions', $this->textdomain), $callback, $tab, $section,
	        array_merge(
				array(
					'id' 	=> 'block',
					'desc' 	=> __('If you set this option to true, a user will not be able to submit unoriginal content. A new CopyScape query will be done on each try. If you set this to false, the submission will be added to the pending queue and the copyscape status will be displayed in a new column (see next option).', $this->textdomain),
					'type' 	=> 'bool'
				),
				$args
			)
		...
		add_settings_field(
	        'column_types', __('Add a CopyScape column for these post types', $this->textdomain), $callback, $tab, $section,
	        array_merge(
				array(
					'id' 	=> 'column_types',
					'desc' 	=> __('The new columns will appear in the admin area.', $this->textdomain),
					'type' 	=> 'multicheckbox',
	...
	public function section_callback($args){
		?>
			<p class="description"><?php _e('In this section you can manage your CopyScape API keys and settings.', $this->textdomain); ?></p>
		<?php
	}
}