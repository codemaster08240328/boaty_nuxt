...
class WPFEPP_Tab_Settings_Email extends WPFEPP_Tab
{
	...
	public function create_options(){
		...
		add_settings_section( $section, __('Emails', $this->textdomain), array($this, 'section_callback'), $tab );

		register_setting( $tab, $option_id, array($this->validator, 'validate') );

	    add_settings_field(
	        'sender_name', __('Sender Name', $this->textdomain), $callback, $tab, $section,
	        array_merge(
				array(
					'desc' 	=> __('This name will be used as the sender name in notification emails.', $this->textdomain),
					'id' 	=> 'sender_name',
					'type' 	=> 'text'
				),
				$args
	    ...
	    add_settings_field(
	        'sender_address', __('Sender Address', $this->textdomain), $callback, $tab, $section,
	        array_merge(
				array(
					'desc' 	=> __('Emails will be sent from this address.', $this->textdomain),
					'id' 	=> 'sender_address',
					'type' 	=> 'text'
				),
				$args
	    ...
	    add_settings_field(
	        'email_format', __('Email Format', $this->textdomain), $callback, $tab, $section,
	        array_merge(
				array(
					'desc' 	=> __('In which format would you like to send the emails.', $this->textdomain),
					'id' 	=> 'email_format',
					'type' 	=> 'select',
					'items' => array('plain' => __('Plain Text', $this->textdomain), 'html' => __('HTML', $this->textdomain))
				),
				$args
			)
	    );
	...
	public function section_callback($args){
		?>
			<p class="description"><?php _e('These are the general email settings of the plugin. You can modify the actual content of the emails by editing your form.', $this->textdomain); ?></p>
		<?php
	}
}