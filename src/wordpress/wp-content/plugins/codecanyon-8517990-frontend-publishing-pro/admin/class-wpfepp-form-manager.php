...
class WPFEPP_Form_Manager
{
	...
	public function __construct($version, $textdomain)
	{
		...
		$fields_tab 		= new WPFEPP_Tab_Form_Fields($this->version, $this->textdomain, 'fields', __('Fields', $this->textdomain), $this);
		$settings_tab 		= new WPFEPP_Tab_Form_Settings($this->version, $this->textdomain, 'settings', __('Settings', $this->textdomain), $this);
		$emails_tab 		= new WPFEPP_Tab_Form_Emails($this->version, $this->textdomain, 'emails', __('Emails', $this->textdomain), $this);
	...
	private function set_defaults(){
		...
		$this->defaults['emails'] = array(
			'user_email_subject' 	=> __("Thank you for your contribution", $this->textdomain),
			'user_email_content' 	=> sprintf(__("Hi %s,\n\nThank you for submitting the article '%s' at our website %s. It has been added to the queue and will be published shortly.\n\nRegards,\n%s\n%s", $this->textdomain), "%%AUTHOR_NAME%%", "%%POST_TITLE%%", "%%SITE_NAME%%", "%%ADMIN_NAME%%", "%%SITE_URL%%"),
			'admin_email_subject' 	=> sprintf(__("A new article has been submitted on your website %s", $this->textdomain), "%%SITE_NAME%%"),
			'admin_email_content' 	=> sprintf(__("Hi %s,\n\nA new article has been added to your website. You can view and edit all your articles here:\n\n%s\n\nRegards,\nYour web server", $this->textdomain), "%%ADMIN_NAME%%", "%%EDIT_LINK%%")
		);
	}
	...
	public function add_menu_item(){
		$this->page_hook = add_menu_page(
		    __('Frontend Publishing Pro', $this->textdomain),
		    __('Frontend Publishing Pro', $this->textdomain),
		    'manage_options',
		    $this->page,
		...
		add_submenu_page( $this->page, __('Frontend Publishing Forms', $this->textdomain), __('Forms', $this->textdomain), 'manage_options', $this->page);
	}
	...
	private function generate_default_form_fields(){
		...
		foreach ($this->post_types as $post_type) {
			$default_fields = array(
					'title' 		=> array( 'type' => 'title', 'widget_label' => __('Title', $this->textdomain), 'label' => __('Title', $this->textdomain), 'enabled' => true, 'required'=> true, 'width' => '', "min_words" => 2, "max_words" => 50, 'strip_tags' => 'all', 'fallback_value' => '', 'prefix_text' => '' ),
					'content' 		=> array( 'type' => 'content', 'widget_label' => __('Content', $this->textdomain), 'label' => __('Content', $this->textdomain), 'enabled' => true, 'required'=> true, 'width' => '', "min_words" => 10, "max_words" => 1000, "max_links" => 2, "element" => 'richtext', 'strip_tags' => 'unsafe', 'nofollow' => false, 'media_button' => true, 'fallback_value' => '', 'prefix_text' => '' ),
					'excerpt' 		=> array( 'type' => 'excerpt', 'widget_label' => __('Excerpt', $this->textdomain), 'label' => __('Excerpt', $this->textdomain), 'enabled' => true, 'required'=> false, 'width' => '', "min_words" => 10, "max_words" => 50, 'strip_tags' => 'all', 'fallback_value' => '', 'prefix_text' => '' ),
					'thumbnail' 	=> array( 'type' => 'thumbnail', 'widget_label' => __('Thumbnail', $this->textdomain), 'label' => __('Thumbnail', $this->textdomain), 'enabled' => true, 'required'=> false, 'width' => '', 'fallback_value' => '', 'prefix_text' => '' ),
					'formatting' 	=> array( 'type' => 'post_format', 'widget_label' => __('Formats', $this->textdomain), 'label' => __('Formats', $this->textdomain), 'enabled' => true, 'required'=> false, 'width' => '', 'fallback_value' => '', 'prefix_text' => '' )
				);
	...
	public function create_form_ajax(){
		...
		if(!wp_verify_nonce($nonce, 'wpfepp-create-form')){
			$response = array('success' => false, 'error' => __('Security check failed!', $this->textdomain));
			die(json_encode($response));
		}
		...
		if($parent_form_id != -1){
			...
			if($parent_form && $parent_form['post_type'] != $post_type ){
				$response = array('success' => false, 'error' => __('You can only import items from a form which has the same post type as the one selected for this form.', $this->textdomain));
				die(json_encode($response));
			}
		}
	...
	private function render_form_list_table(){
		?>
			...
			<input type="hidden" name="page" value="<?php echo $this->page; ?>" />
				<?php
					$table = new WPFEPP_List_Table($this->version, $this->textdomain);
					$table->prepare_items();
					$table->display();
				?>
			</form>
	...
	public function admin_notices() {
    	...
    	?>
		    <div class="updated">
		        <p><?php _e('Form(s) deleted successfully!', $this->textdomain); ?></p>
		    </div>
	    <?php
		}
    	...
    	?>
		    <div class="updated">
		        <p><?php _e('The form has been updated successfully!', $this->textdomain); ?></p>
		    </div>
	    <?php
		}
	}
	...
	public function display_form_selector(){
		...
		<select name="parent_form">
			<option value="-1"><?php _e('None', $this->textdomain); ?></option>
