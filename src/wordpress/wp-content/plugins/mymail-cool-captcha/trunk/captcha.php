...
Plugin URI: http://rxa.li/mymail?utm_source=MyMail+Cool+Captcha+for+Forms
...
?>
	<table class="form-table">
		...
		<tr valign="top">
			...
			<td><label><input type="checkbox" name="mymail_options[coolcaptcha_loggedin]" value="1" <?php checked(mymail_option('coolcaptcha_loggedin')); ?>> <?php _e('disable the captcha for logged in users', 'mymail_coolcaptcha') ?></label></td>
		</tr>
		<tr valign="top">
		...
		<tr valign="top">
			...
			<td>
				<?php
				echo sprintf('<br><img src="%s" width="'.mymail_option('coolcaptcha_width').'" height="'.mymail_option('coolcaptcha_height', 70).'" style="border:1px solid #ccc">', add_query_arg(array(
		...
		<tr valign="top">
			...
			<td><label><input type="checkbox" name="mymail_options[coolcaptcha_blur]" value="1" <?php checked(mymail_option('coolcaptcha_blur')); ?>> <?php _e('use blur', 'mymail_coolcaptcha') ?></label></td>
		</tr>
		<tr valign="top">
			<th scope="row"><?php _e('Line' ,'mymail_coolcaptcha'); ?></th>
			<td><label><input type="checkbox" name="mymail_options[coolcaptcha_line]" value="1" <?php checked(mymail_option('coolcaptcha_line')); ?>> <?php _e('strike out the text', 'mymail_coolcaptcha') ?></label></td>
		</tr>
		<tr valign="top">
...
<?php
	...
	public function coolcaptcha_img(){
		...
		$captcha->blur = !!mymail_option('coolcaptcha_blur');
	...
	public function check_captcha($object){
		...
		if(is_user_logged_in() && mymail_option('coolcaptcha_loggedin')) return $object;

		$formid = (isset($_POST['formid'])) ? intval($_POST['formid']) : 0;

		if(!in_array($formid, mymail_option('coolcaptcha_forms', array()))) return $object;
	...
	public function check_captcha_v1($errors){
		...
		if(is_user_logged_in() && mymail_option('coolcaptcha_loggedin')) return $errors;

		$formid = (isset($_POST['formid'])) ? intval($_POST['formid']) : 0;

		if(!in_array($formid, mymail_option('coolcaptcha_forms', array()))) return $errors;
