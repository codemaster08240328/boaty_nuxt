            ...
            <v-card-text>
              <v-container grid-list-md>
                <v-layout wrap>
                  ...
                  <v-flex xs12>
                    Contact no. {{ editedItem.phone_number }}
                  </v-flex>
                  <v-flex xs12>
                    Message: {{ editedItem.message }}
                  </v-flex>
                  <v-flex xs12>
                    booked at {{ editedItem.created_at }}
                  </v-flex>
...
export default {
  methods: {
    ...
    async deleteItem (item) {
      ...
      await this.$axios.post(`/api/v1/sc-secret-admin/bookings/delete/${item.id}`)
  ...
  async asyncData ({ app }) {
    let { data } = await app.$axios.get('/api/v1/sc-secret-admin/bookings')
    return {
      items: data
    }
  },
