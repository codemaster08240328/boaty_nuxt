    ...
    searchBoats () {
      ...
      this.$nextTick(function () {
        if (that.query.region) {
          ...
          setTimeout(async () => {
            let searchCookie = that.$interaction.getDataFromCookie({ name: 'search' })
            let searchRegion = searchCookie.region.split('/')
            ...
            each(searchRegion, (val) => {
              if (val !== '') {
                if (step === 0) {
                  searchLocation.country = val
                }

                if (step === 1) {
                  searchLocation.area = val
                }

                if (step === 2) {
                  searchLocation.base = val
                }
                step++
              }
            })
            console.log('searchlocation', searchLocation)
            console.log('cookie', that.$cookie.get('search'))
            this.$emit('updateParams', searchLocation)
            that.$interaction.setDataInCookies({ name: 'search', data: searchLocation })

            try {
              const { data } = await this.$axios.post(
                '/api/v1/search/boats/filter',
                that.$interaction.getDataFromCookie({ name: 'search' })
              )
